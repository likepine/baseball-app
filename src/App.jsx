...
const firebaseConfig = {
  apiKey: "AIzaSyCB-94gFSBLkbXLk_aolwzrh0xD7kWroRI",
  authDomain: "baseball-app-f803d.firebaseapp.com",
  projectId: "baseball-app-f803d",
  storageBucket: "baseball-app-f803d.firebasestorage.app",
  messagingSenderId: "441561956051",
  appId: "1:441561956051:web:174a6c2261f93cafd31e26",
  measurementId: "G-XQV3X8B9LB"
};
...
export default function App() {
  ...
  const parseSMS = async () => {
    ...
    try {
      ...
      if (duplicateRes) {
        if (user) {
          const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'reservations', duplicateRes.id);
      ...
      if (user) {
        const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'reservations', newId);
  ...
  const toggleDateStatus = async (date) => {
    ...
    if (user) {
      const updatePromises = targetReservations.map(res => {
        const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'reservations', res.id);
